    ...
    private void init() {
        ...
        for (Map.Entry<Key<?>, Binding<?>> bindingEntry : bindings.entrySet()) {
            ...
            if (!bindingKey.hasAttributes() && bindingKey.getTypeLiteral().getType() instanceof Class
                    && !((Class<?>) bindingKey.getTypeLiteral().getType()).isAnnotationPresent(IgnoreAfterburner.class)) {
                ...
                logger.trace("Registered instance instance {} for class {} in afterburner", instance.getClass()

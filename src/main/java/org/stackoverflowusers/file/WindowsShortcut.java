 ...
 * 
 * Written by: (the stack overflow users, obviously!) Headerified, refactored and commented by Code Bling
 * http://stackoverflow.com/users/675721/code-bling Network file support added by Stefan Cordes
 * http://stackoverflow.com/users/81330/stefan-cordes Adapted by Sam Brightman
 * http://stackoverflow.com/users/2492/sam-brightman Based on information in 'The Windows Shortcut File Format' by Jesse
 * Hager &lt;jessehager@iname.com&gt; And somewhat based on code from the book 'Swing Hacks: Tips and Tools for Killer
 * GUIs' by Joshua Marinacci and Chris Adamson ISBN: 0-596-00907-0 http://www.oreilly.com/catalog/swinghks/
 */
...
public class WindowsShortcut {
	...
	/**
	 * Provides a quick test to see if this could be a valid link ! If you try to instantiate a new WindowShortcut and
	 * the link is not valid, Exceptions may be thrown and Exceptions are extremely slow to generate, therefore any code
	...
	public static boolean isPotentialValidLink(FileObject file) throws IOException {
		...
		try {
			isPotentiallyValid = file.getType().equals(FileType.FILE)
					&& file.getName().getExtension().toLowerCase().equals("lnk") && fis.available() >= minimum_length
